...
const SellActionWindow = ({ uid }) => {
  ...
  const handleSell = useCallback(async () => {
    ...
    try {
      ...
      setStatus("success");
      setTimeout(() => generalContext.closeSellWindow(), 1200);
    } catch (e) {
      setStatus("error");
  ...
  const handleKeyDown = useCallback((e) => {
    if (e.key === "Escape") generalContext.closeSellWindow();
    if (e.key === "Enter" && !loading) handleSell();
  }, [qty, price, loading, generalContext, handleSell]);
    ...
    <>
      ...
      <div className="ag-action-window" style={{
        ...
        <div style={{
          ...
          <button onClick={() => generalContext.closeSellWindow()} style={{
            ...
            <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
              {ORDER_TYPES.map((t) => (
                <button key={t} onClick={() => setOrderType(t)} style={{
                  ...
                  border: "1px solid",
                  borderColor: orderType === t ? "var(--red-primary)" : "var(--border)",
                  background: orderType === t ? "var(--red-pale)" : "transparent",
                  color: orderType === t ? "var(--red-primary)" : "var(--text-muted)",
            ...
            <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
              {PRODUCT_TYPES.map((t) => (
                <button key={t} onClick={() => setProductType(t)} style={{
                  ...
                  border: "1px solid",
                  borderColor: productType === t ? "var(--red-primary)" : "var(--border)",
                  background: productType === t ? "var(--red-pale)" : "transparent",
                  color: productType === t ? "var(--red-primary)" : "var(--text-muted)",
                  transition: "all 0.15s",
                }}>
                  {t}
                </button>
            ...
            <div style={{ display: "flex", gap: "0.4rem" }}>
              {VALIDITY.map((v) => (
                <button key={v} onClick={() => setValidity(v)} style={{
                  ...
                  fontSize: "0.68rem", fontWeight: 600, cursor: "pointer",
                  border: "1px solid",
                  borderColor: validity === v ? "var(--red-primary)" : "var(--border)",
                  background: validity === v ? "var(--red-pale)" : "transparent",
                  color: validity === v ? "var(--red-primary)" : "var(--text-muted)",
                  transition: "all 0.15s",
                }}>
                  {v}
                </button>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <button onClick={() => generalContext.closeSellWindow()} className="ag-btn ag-btn--ghost ag-btn--sm">
              Cancel
            </button>
            <button
              onClick={handleSell}
              ...
              style={{
                background: loading ? "rgba(192,57,43,0.7)" : "var(--red-primary)",
