...
const Summary = () => {
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {[200, 160, 200, 160].map((w, i) => (
        <div key={i} className="ag-skeleton" style={{ height: "18px", width: w, borderRadius: "6px" }} />
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div>
        <h2 style={{
          fontFamily: "var(--font-ui)", fontSize: "1.25rem",
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span className="ag-badge ag-badge--green" style={{ fontSize: "0.6rem" }}>Live</span>
        </div>

        <div style={{ padding: "1.25rem", display: "flex", gap: "1.5rem", alignItems: "stretch" }}>

          {/* Main figure */}
          <div style={{ flex: "0 0 auto" }}>
            <div style={{
              fontFamily: "var(--font-mono)", fontSize: "2rem",
              fontWeight: 700, color: "var(--green-primary)",
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.5rem", justifyContent: "center" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>Margins used</span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", fontWeight: 600, color: "var(--text-primary)" }}>
                ₹{fmt(funds?.used_margin || 0)}
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>Opening balance</span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", fontWeight: 600, color: "var(--text-primary)" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            <span className="ag-badge ag-badge--neutral" style={{ fontSize: "0.6rem" }}>
              {holdings.length}
            </span>
          </div>
        </div>
        ...
        {holdings.length === 0 ? (
          <div className="ag-empty" style={{ padding: "2rem" }}>
            ...
            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>No holdings yet</div>
          </div>
        ) : (
          <div style={{ padding: "1.25rem", display: "flex", gap: "1.5rem", alignItems: "stretch" }}>
            ...
            <div style={{ flex: "0 0 auto" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: "0.5rem" }}>
                <div style={{
                  ...
                  color: isPnLProfit ? "var(--green-primary)" : "var(--red-primary)",
                }}>
                ...
                <div style={{
                  ...
                  borderRadius: "4px",
                  background: isPnLProfit ? "var(--green-pale)" : "var(--red-pale)",
                  color: isPnLProfit ? "var(--green-primary)" : "var(--red-primary)",
                }}>
                  {isPnLProfit ? "+" : ""}{totalPnLPct.toFixed(2)}%
                </div>
              </div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "0.35rem" }}>
                P&amp;L
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.5rem", justifyContent: "center" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>Current value</span>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", fontWeight: 600, color: "var(--text-primary)" }}>
                  ₹{fmt(totalCurrValue)}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>Investment</span>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", fontWeight: 600, color: "var(--text-primary)" }}>
                  ₹{fmt(totalInvestment)}
                </span>
              </div>
      ...
      <div className="ag-grid-2">
        {[
          { label: "Orders",    sub: "View order history",   to: "/orders",    icon: "📋", badge: null             },
          { label: "Positions", sub: "Open intraday trades", to: "/positions", icon: "📊", badge: null             },
          { label: "Holdings",  sub: `${holdings.length} stocks`,   to: "/holdings",  icon: "📦", badge: holdings.length },
          { label: "Funds",     sub: "Manage balance",       to: "/funds",     icon: "💰", badge: null             },
        ].map((item) => (
          <a key={item.to} href={item.to} style={{ textDecoration: "none" }}
            onClick={(e) => { e.preventDefault(); window.location.href = item.to; }}
          >
            <div style={{
              background: "var(--bg-surface)",
              border: "1px solid var(--border)",
              ...
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--green-light)"; e.currentTarget.style.boxShadow = "var(--shadow-md)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
            ...
            >
              ...
              {item.badge !== null && (
                <span className="ag-badge ag-badge--neutral" style={{ fontFamily: "var(--font-mono)" }}>
                  {item.badge}
                </span>
              )}
